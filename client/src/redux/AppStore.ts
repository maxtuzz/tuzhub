...
function* rootSaga() {
  yield all([fork(notificationsSaga), fork(apiEntrySaga), fork(apiFormSaga), fork(apiSpecSaga)]);
}
...
export const AppStore = createStore(rootReducer, composeWithDevTools(middleware));
