...
const SchemaList: React.FC<Props> = ({ components, navPath }) => {
  ...
  const refs =
    entries &&
    entries.reduce((prevRefs: SectionRefs, [key]) => {
  ...
  return (
    <ObjectsContainer>
      ...
      <ObjectListContainer>
        {entries &&
          entries.map(([schemaName, resource]) => {
            const schema = resource as OpenAPIV3.SchemaObject;

            return (
              <div ref={refs && refs[schemaName]} key={schemaName}>
                <SchemaAccordion schemaName={schemaName} schema={schema} openWhen={schemaName === navPath} />
              </div>
            );
          })}
      </ObjectListContainer>
