...
const ApiList: React.FC<ApiListProps> = ({ apiEntries, isLoading, getApis, resetSpec }) => {
  ...
  const refreshList = (searchTerm: string) => {
    setFilteredApis(
      apiEntries.filter(apiEntry => {
        return (
          apiEntry.displayName.includes(searchTerm) ||
  ...
  return (
    <Container>
      {!isLoading && (
        <SearchBar
          defaultText={searchQuery != null ? searchQuery : undefined}
      ...
      {isLoading ? (
        ...
        <ListContainer>
          {filteredApis?.map((apiEntry, index) => (
            <ApiCard key={index} apiEntry={apiEntry} fadeInSeconds={0.1 * (index + 1)} />
          ))}
        </ListContainer>
      )}
    </Container>
