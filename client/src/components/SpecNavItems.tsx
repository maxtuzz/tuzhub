...
const SpecNavItems: React.FC<Props & Functions> = ({ apiDoc, specLoading, scrollTo }) => {
  ...
  const resources =
    apiDoc?.paths &&
    ResourceFormatter.getPaths(apiDoc?.paths).map(path => (
  ...
  return (
    <NavItemsContainer>
      ...
      <ExpandableContent open={resourcesOpen}>{resources}</ExpandableContent>
      ...
      <ExpandableContent open={objectsOpen}>{objectSchemas}</ExpandableContent>
    </NavItemsContainer>
  );
};
